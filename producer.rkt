  ...
  (try
   (let loop ([i 0])
    (if (equal? i 10000)
        'DONE
        (let ([key (uuid-string)])
          (produce producer "foobar"
                   key (format "uuid ~a produced at ~a ms" key (current-inexact-monotonic-milliseconds)))
          (sleep 0.5)
          (loop (add1 i)))))
   (catch (exn:fail? e)
     (displayln (format "bummer! got ~a" e)))
